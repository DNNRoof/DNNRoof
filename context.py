...
class RooflineContext():
    def __init__(self,
        ...
        import onnx
        self.onnx_model = str(_tmp() / 'roofline_test_model.onnx')
...
class ModelContext():
    def __init__(self, subjects: list, collected_data: CollectedData, model_backend: _BaseBackend, onnx_model: str, batch_size_list: list, repeat_count: int, backend_options: str, data_width: Tuple[int, int], llc_reuse_size: float, inputs_shape_override: Dict[str, List[Union[int, None]]]) -> None:
        ...
        if inputs_shape_override:
            ...
            self.onnx_model = str(_tmp() / 'inputs_shape_override_model.onnx')
            if os.path.isfile(str(_tmp() / 'model_external_data.pb')):
                os.unlink(str(_tmp() / 'model_external_data.pb'))
            ...
            for batch_size in self.batch_size_list:
                ...
                if 'model.bench.e2e_prof' in self.subjects:
                    if 'e2e_prof' not in self.model_backend_env.supported:
                        ...
                        times = self.model_backend_env.time_run(self.repeat_count)
                        ...
                        if batch_data.better_total_flops and not os.getenv('DNNROOF_E2E_NOT_USE_LAYER_DATA'):
                        ...
                        batch_data.flops_std = np.std(model_flops / times)
                        ...
                        if batch_data.better_total_memory and not os.getenv('DNNROOF_E2E_NOT_USE_LAYER_DATA'):
                        ...
                        batch_data.memory_std = np.std(model_memory_access / times)
