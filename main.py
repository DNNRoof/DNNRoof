...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog = "DNNRoof",
        description = "DNNRoof is designed to be a universal, end-to-end, fine-grained AI model inference performance analysis and hardware AI benchmark tool",
    ...
    parser.add_argument('-B', '--model-backend', default='trtexec', help="backend to run model, available: %s"
        % '|'.join(model.backend.backends))
    ...
    if args.model_backend not in model.backend.backends:
    ...
    tmpdir = os.getenv('DNNROOF_TMPDIR') or str(Path(tempfile.gettempdir()) / 'dnnroof')  # default like /tmp/dnnroof
    try:
        Path(tmpdir).mkdir(parents=True, exist_ok=True)
        assert os.access(tmpdir, os.W_OK)
    except Exception as e:
        log.error('tmpdir %s is not writeable (%s), you can set it in DNNROOF_TMPDIR env var', tmpdir, e)
        sys.exit(1)
    os.environ['DNNROOF_TMPDIR'] = tmpdir
