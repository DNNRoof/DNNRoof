...
def _ncu_get_flops_fallback(kernel_data: dict, data_width: int):
    ...
    if os.getenv("DNNROOF_BACKEND_TRTEXEC_NCU_IGNORE_FALLBACK"):
...
def layer_prof_ncu(obj: Trtexec, batch_size: int) -> dict:
    # use times from ncu instead of trtexec, times from ncu is a little bit slow than trtexec
    USE_TIMES_FROM_NCU = os.getenv('DNNROOF_BACKEND_TRTEXEC_NCU_USE_TIMES_FROM_NCU')

    # stage1: trtexec to get layer names
    tmp_profile_file = str(_tmp() / "trtexec_profile.txt")
    ...
    ncu_output_file = str(_tmp() / ("%s.csv" % output_name))
    ncu_rep = "-o " + output_name if os.getenv('DNNROOF_BACKEND_TRTEXEC_NCU_SAVE_NCU_REP') else ""
    ncu_set = "--set=roofline"
    if os.getenv('DNNROOF_BACKEND_TRTEXEC_NCU_FULL_SET'):
        ncu_set = "--set=full \
    ...
    # NCU #
    if not os.getenv('DNNROOF_BACKEND_TRTEXEC_SKIP_NCU_RUN'):     # TODO: debug flag, remove in release
