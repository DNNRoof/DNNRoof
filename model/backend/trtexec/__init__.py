...
class Trtexec(_BaseBackend):
    ...
    def pre_batch_run(self, batch_size: int) -> None:
        ...
        if any(type(x[0]) is int for x in inputs.values()):
            ...
            change_inputs_batch_size(m, None)
            onnx_model = str(_tmp() / "dyn.onnx")
        ...
        self.target_model = str(_tmp() / "model.trt")
        ...
        if not os.getenv('DNNROOF_BACKEND_TRTEXEC_NO_PROFILING_VERBOSITY'):
        ...
        # CONVERT #
        if not os.getenv('DNNROOF_BACKEND_TRTEXEC_SKIP_CONVERT'):     # TODO: debug flag, remove in release
            ...
            if trtexec_convert_run.returncode != 0:
                ...
                if "Unknown option: --profilingVerbosity" in trtexec_convert_run.stderr.decode():
                    log.error("if you are using old version of TensorRT, try to set 'DNNROOF_BACKEND_TRTEXEC_NO_PROFILING_VERBOSITY'")
    ...
    def time_run(self, repeat: int = 10, warm_up = None) -> np.ndarray:
        ...
        tmp_time_file = str(_tmp() / "trtexec_times.txt")
    ...
    def layer_prof(self, batch_size: int) -> List[ModelBenchBatchLayerData]:
        ...
        tmp_profile_file = str(_tmp() / "trtexec_profile.json")
        tmp_layer_info_file = str(_tmp() / "trtexec_layer_info.json")
        trtexec_inference_cmd = [self.trtexec_bin,
            "--warmUp=1000",
            "--loadEngine="+self.target_model,
                ...
                if layer['Name'] not in dummy_layers:  # not alias layer in previous pass
                    ...
                    if layer['LayerType'] == 'Myelin':
                        ...
                        if self.layer_prof_use_nsys_for_myelin:
                            ...
                            total_layer_time = 0
                            not_matched_time = 0
                            for sub_layer, time in sub_names:
    ...
    def layer_prof_ncu(self, batch_size: int) -> dict:
