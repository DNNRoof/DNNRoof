...
def nsys_run(obj: Trtexec) -> str:
    tmp_profile_file = str(_tmp() / 'nsys_profile.nsys-rep')
    tmp_json_file = str(_tmp() / 'nsys_profile.json')
...
# return [(sub_name, median_time), ...]
def nsys_myelin_layer_get_names(json_file: str, myelin_layer_name: str, skip: int = 10) -> List[Tuple[str, float]]:
                ...
                if 'NvtxEvent' in sub_e and 'TextId' in sub_e['NvtxEvent']:
                    ...
                    if start <= sub_start and sub_end <= end:
                        ...
                        for correlation_id in correlation_ids:
                            ...
                            results_dict[name].append(time_kernel)
    ...
    for name in results_name:
        results.append((name, np.median(results_dict[name])))
