...
class ONNXRuntimeBackend(_BaseBackend):
    ...
    def time_run(self, repeat: int = 10, warm_up = 3) -> np.ndarray:
        ...
        sess_options.profile_file_prefix = str(_tmp() / ('onnxruntime_profile_' + Path(self.onnx_model).stem))
    ...
    def layer_prof(self, batch_size: int) -> List[ModelBenchBatchLayerData]:
        ...
        sess_options.graph_optimization_level = self.graph_optimization_level
        sess_options.profile_file_prefix = str(_tmp() / ('onnxruntime_profile_' + Path(self.onnx_model).stem))
        sess_options.optimized_model_filepath = str(_tmp() / ('onnxruntime_optimized_model_' + Path(self.onnx_model).name))
